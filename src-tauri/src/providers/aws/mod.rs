...
impl DNSProvider for AwsProvider {
    ...
    async fn update_record(&self, _domain: &str, _record_id: &str, _new_content: &str) -> Result<UpdateResult> {
        Err(AppError::Provider(ProviderError::Unknown(
            "AWS Route53 提供商尚未实现".to_string()
        )))
    }

    async fn create_record(&self, _domain: &str, _record_name: &str, _record_type: DNSRecordType, _content: &str) -> Result<DNSRecord> {
        Err(AppError::Provider(ProviderError::Unknown(
            "AWS Route53 提供商尚未实现".to_string()
        )))
    }
